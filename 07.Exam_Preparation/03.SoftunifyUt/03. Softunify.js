...
describe('SoftUniFy', () => {
    ...
    beforeEach(function () {
        testClass = new SoftUniFy();
    });

    it('should allSongs is an object', () => {
        assert.deepEqual(testClass.allSongs, {})
    });

    it('should empty song list return message if song is not downloaded', function () {
        assert.equal(testClass.playSong('song'),
            'You have not downloaded a song song yet. ' +
            'Use SoftUniFy\'s function downloadSong() to change that!')
    });

    it('should rateArtist return message if artist not in the list ', function () {
        assert(testClass.rateArtist('artist'),
